...
`GET /channel/:id` → `200` 公開チャンネル JSON

## 退出

`POST /channel/:id/leave` → `204`

管理者は退出不可（`403 channel_admin_cannot_leave`）。退出時 `channel:participant:left` を EventBus 経由で配信。

## 削除

`DELETE /channel/:id` → `204`

管理者のみ。論理削除。`channel:deleted` を配信。

## Socket イベント（EventBus）

| イベント | タイミング |
|----------|------------|
| `channel:participant:joined` | 新規 join 成功時 |
| `channel:participant:left` | leave 成功時 |
| `channel:deleted` | delete 成功時 |
